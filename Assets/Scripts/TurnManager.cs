...
public class TurnManager : MonoBehaviour
{
    ...
    {
        if (GameManager.Instance == null || GameManager.Instance.encounteredEnemyPrefabs.Count == 0)
        {
            Debug.LogWarning("[TurnManager] GameManager에 적 데이터가 없습니다. 에디터에 배치된 테스트용 적을 유지합니다.");
            return;
        }

        // 혹시 에디터에 미리 올려둔 적이 있다면 리스트 비우기 (중복 방지)
        enemyParty.Clear();

        var enemyPrefabs = GameManager.Instance.encounteredEnemyPrefabs;

        for (int i = 0; i < enemyPrefabs.Count; i++)
        {
            if (i >= enemySpawnPositions.Count) break;

            GameObject prefab = enemyPrefabs[i];
            Transform spawnPoint = enemySpawnPositions[i];

            // 지정된 위치에 적 프리팹 생성
            GameObject spawnedEnemy = Instantiate(prefab, spawnPoint.position, spawnPoint.rotation);
            BattleUnit unit = spawnedEnemy.GetComponent<BattleUnit>();

            if (unit != null)
            {
                unit.isPlayerTeam = false; // 아군이 아님을 명시
                enemyParty.Add(unit);
            }
        }
        Debug.Log($"<color=red>[시스템] GameManager로부터 {enemyParty.Count}명의 적군을 성공적으로 소환했습니다.</color>");
    }
